...
SRR_FOLDER = "single_round_results"


def store_results(benchmark_name, task, posteriors, samples, compute_predictives=True, single_round_results=None):
    ...
    if not os.path.exists(path + SEPERATOR + folder_name + SEPERATOR + SRR_FOLDER):
        os.mkdir(path + SEPERATOR + folder_name + SEPERATOR + SRR_FOLDER)
    ...
    with open(path + SEPERATOR + folder_name + SEPERATOR + SRR_FOLDER + SEPERATOR + f"single_round_results.pkl", "wb") as f:
        import pickle
        pickle.dump(single_round_results, f)
