...
def _test_jax_installation_unsafe(device: Literal["cpu", "gpu"]):
    ...
    assert any(d.platform == device for d in jax.local_devices())
    print(" OK.")
...
def test_jax_installation(device, verbose=False):
    try:
        ...
        if not verbose:
            print(
                "Run python -m jax_utils.test_jax_installation --verbose to get more information."
            )
        else:
...
if __name__ == "__main__":
    ...
    test_jax_installation(device="cpu" if args.cpu else "gpu", verbose=args.verbose)
