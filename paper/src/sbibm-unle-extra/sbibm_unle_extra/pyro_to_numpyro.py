...
def _convert_inverse_transform(
    t: tdist.transforms._InverseTransform,  # pyright: ignore [reportPrivateUsage]
    implementation: _DISTMODULE_TARGETS_T,
) -> np_transforms._InverseTransform:  # pyright: ignore [reportPrivateUsage]
    if implementation == "numpyro":
...
_register_transforms_converter(
    _convert_inverse_transform,
    tdist.transforms._InverseTransform,  # pyright: ignore [reportPrivateUsage]
)
