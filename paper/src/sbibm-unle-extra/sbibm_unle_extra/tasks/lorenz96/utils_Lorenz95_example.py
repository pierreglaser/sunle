import numpy as np
from abcpy.probabilisticmodels import Continuous, InputConnector, ProbabilisticModel
...
def extract_params_from_journal_Lorenz95(jrnl, step=None):
    ...
    return np.concatenate(
        (
            np.array(params["theta1"]).reshape(-1, 1),
            np.array(params["theta2"]).reshape(-1, 1),
            np.array(params["sigma_e"]).reshape(-1, 1),
            np.array(params["phi"]).reshape(-1, 1),
        ),
        axis=1,
    )
...
class StochLorenz95(ProbabilisticModel, Continuous):
    """Generates time dependent 'slow' weather variables following forecast
    model of Wilks [1], a stochastic reparametrization of original Lorenz model
    Lorenz [2].

    [1] Wilks, D. S. (2005). Effects of stochastic parametrizations in the
    lorenz ’96 system.
    Quarterly Journal of the Royal Meteorological Society, 131(606), 389–407.

    [2] Lorenz, E. (1995). Predictability: a problem partly solved. In Proceedings
    of the Seminar on Predictability, volume 1, pages 1–18. European Center on
    Medium Range Weather Forecasting, Europe

    Parameters
    ----------
    parameters: list
        Contains the closure parameters of the parametrization:
        [theta_1, theta_2, sigma_e, phi].
    initial_state: numpy.ndarray, optional
        Initial state value of the time-series, The default value is None,
        which assumes a previously computed
    ...
    time_units: float, optional
        Number of time unites for which to run the system; default to 4, which
        corresponds roughly to 20 days.
    ...
    def __init__(
        ...
        self.F = F  # sometimes this parameter is given value 20; it is given value 10
        ...
        self.b = b
        self.h = h
        self.c = c  # 10 is also used sometimes; it corresponds to the easy case,
        # while c=4 is harder.
        self.J = J
        self.K = K
        ...
        if len(parameters) != 4:
            raise RuntimeError(
                "Input list must be of length 4, "
                "containing [theta1, theta2, sigma_e, phi]."
            )

        input_connector = InputConnector.from_list(parameters)  # type: ignore
        super().__init__(input_connector, name)

    def _set_initial_state(self, initial_state, lam=0):
        """This sets the initial state to a fixed value (if that was not provided).
        lam is the magnitude of the gaussian
        noise that is added to the initial state; it is used only in the Ensemble model.
        """
        if initial_state is None:
            # Assign initial state; this has 40 variables, it is suitable for K=40.
            # In case K<40 is used, the initial
        ...
        if lam > 0:
            self.initial_state += np.random.normal(
                scale=lam, size=self.initial_state.shape
            )
    ...
    def _check_input(self, input_values):
        ...
        # why? this does not make much sense, the parameters of the deterministic
        # part could be smaller than 0
        #    return False
    ...
    def forward_simulate(
        self, input_values, k, rng=np.random.RandomState(), mpi_comm=None
    ):
        assert (
            mpi_comm is None
        ), "MPI parrelization not supported in `forward_simulate`."
    ...
    def forward_simulate_true_model(
        self, k, n_timestep_per_time_unit=None, rng=np.random.RandomState()
    ):
        # Do the actual forward simulation
        vector_of_k_samples_x, vector_of_k_samples_y = self.Lorenz95True(
            k, n_timestep_per_time_unit=n_timestep_per_time_unit, rng=rng
        )
        # Format the output to obey API
    ...
    def Lorenz95(self, theta1, theta2, sigma_e, phi, k, rng=np.random.RandomState()):
        # Generate n_simulate time-series of weather variables
        # satisfying Lorenz 95 equations
        result = []
        ...
        for k in range(0, k):
            ...
            eta = (
                sigma_e
                * np.sqrt(1 - pow(phi, 2))
                * rng.normal(0, 1, self.initial_state.shape[0])
            )

            # Initialize the time-series
            timeseries = np.zeros(shape=(self.K, self.n_timestep), dtype=np.float32)
            ...
            for ind in range(0, self.n_timestep - 1):
                ...
                x = self._rk4ode(
                    self._l95ode_par,
                    np.array([time_steps[ind], time_steps[ind + 1]]),
                    timeseries[:, ind],
                    parameter,
                )
    ...
    def Lorenz95True(
        ...
        """ "Note that here there is randomness in the choice of the starting value of
        the y variables. I chose them to
        ...
        # Generate n_simulate time-series of weather variables satisfying
        # Lorenz 95 equations
        result_X = []
        result_Y = []

        # Initialize timesteps
        # it is better to use a smaller timestep for the true model, as the solver
        # may diverge otherwise.
        time_steps = np.linspace(0, self.time_units, n_timestep)

        # define the initial state of the Y variables. We take self.J fast variables
        # per slow variable
        ...
        for k in range(0, k):
            ...
            # Initialize the time-series
            timeseries_X = np.zeros(shape=(self.K, n_timestep), dtype=np.float32)
            timeseries_X[:, 0] = self.initial_state[0 : self.K]

            timeseries_Y = np.zeros(
                shape=(self.initial_state_Y.shape[0], n_timestep), dtype=np.float32
            )  # type: ignore
            timeseries_Y[:, 0] = self.initial_state_Y

            # Compute the timeseries for each time steps
            # the loop would not be needed if we wrote a single ode
            # function for both set of variables.
            for ind in range(0, n_timestep - 1):
                ...
                x = self._rk4ode(
                    ...
                    timeseries_X[:, ind],
                    [timeseries_Y[:, ind]],
                )  # we pass the value of the other set of variables as the parameter.
                y = self._rk4ode(
                    self._l95ode_true_Y,
                    np.array([time_steps[ind], time_steps[ind + 1]]),
                    timeseries_Y[:, ind],
    ...
    def _l95ode_true_X(self, t, x, parameter):
        ...
        dx[0] = (
            ...
            - x[0]
            + self.F
            - self.hc_over_b * np.sum(y[0 : self.J])
        )
        dx[1] = (
            -x[-1] * x[0]
            + x[0] * x[2]
            - x[1]
            + self.F
            - self.hc_over_b * np.sum(y[self.J : 2 * self.J])
        )
        for ind in range(2, x.shape[0] - 2):
            dx[ind] = (
                -x[ind - 2] * x[ind - 1]
        ...
        dx[-1] = (
            ...
            - x[-1]
            + self.F
            - self.hc_over_b * np.sum(y[-1 * self.J :])
        )
    ...
    def _l95ode_true_Y(self, t, y, parameter):
        ...
        for ind in range(y.shape[0] - 3):
            dy[ind] = (
                -self.cb * y[ind + 1] * (y[ind + 2] - y[ind - 1])
                - self.c * y[ind]
                + self.hc_over_b * x[ind // self.J]
            )  # // for the integer division.

        dy[-2] = (
            -self.cb * y[-1] * (y[0] - y[-3])
            - self.c * y[-2]
            + self.hc_over_b * x[-2 // self.J]
        )  # // for the integer division.

        dy[-1] = (
            -self.cb * y[0] * (y[1] - y[-2])
            - self.c * y[-1]
            + self.hc_over_b * x[-1 // self.J]
        )  # // for the integer division.
    ...
    def _rk4ode(self, ode, timespan, timeseries_initial, parameter):
        ...
        -------
        np.ndarray
            Timeseries initiated at timeseries_init and satisfying ode
