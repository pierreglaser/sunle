...
def train(cfg, round_):
    ...
    if round_ > 1 or cfg.retrain_first_round:
        ...
        l5pc_directory = Path(list(l5pc.__path__)[0])
        base = l5pc_directory.parent / "results"/ "p31_2"/"multiround"
        with open(join(base, "2022_05_04__08_18_59/inference_r1.pkl"), "rb") as handle:
...
def evaluate(cfg, round_):
    ...
    if not use_dask:
        ...
        for i in range(num_retries):
            try:
                ...
                cluster.scale(cfg.cores)
                client = Client(cluster, timeout=300.0)  # pyright: ignore [reportGeneralTypeIssues]
                futures = client.map(simulate, batches)
                x_list = client.gather(futures)
                client.close()
                cluster.close()
            except Exception as e:
                log.info(f"Exception {e} in round {round_}, retrying...")
                if i == num_retries - 1:
                    raise e
                else:
                    time.sleep(5)
    ...
    with open(f"theta_eval_r{round_}.pkl", "wb") as handle:
        pickle.dump(theta, handle)

    with open(f"xs_eval_r{round_}.pkl", "wb") as handle:
        pickle.dump(stats, handle)

    # save fraction of valid sims in a txt file
    np.savetxt(
        f"fraction_valid_sims_r{round_}.txt",
        np.array([np.sum(valid) / cfg.num_predictives]),
        fmt="%10.10f",
    )

    # there is no .matplotlibrc file to be found???
#     with mpl.rc_context(
#         fname="/tmp/unle/tsnpe_neurips/l5pc/.matplotlibrc"
#     ):
...
def load_pyloric_posterior_from_file(path, round_):
    ...
    l5pc_directory = Path(list(l5pc.__path__)[0])
    base = l5pc_directory.parent / "results" /"p31_4"/"multiround"
