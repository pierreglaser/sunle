...
class SMCParticleApproximation(ParticleApproximation):
    ...
    def from_npdistribution(
        ...
        return cls(
            ...
            log_Z=0.0,
            log_prob=maybe_wrap(
                dist.log_prob  # pyright: ignore[reportGeneralTypeIssues]
            ),
        )
    ...
    def reweight_and_update_Z(self: Self, new_log_prob: LogDensity_T) -> Self:
        ...
        _Z_log_space = self._Z_log_space + cast(
            Numeric, logsumexp(log_weights_unnormalized)
        )
        ...
        def V(x):
            return jnp.nan_to_num(
                new_log_prob.final_log_prob(x) - new_log_prob.init_log_prob(x),
                neginf=-10000000000.0,
            )

        average_potential_new = new_samples.average_of(V)
        average_potential_old = self.average_of(V)
        log_Z_inc = (
            (new_log_prob.beta - prev_log_prob.beta)
            * 0.5
            * (average_potential_new + average_potential_old)
        )
...
def _maybe_resample(
    ...
    key: KeyArray,
    mcmc_state: State_T,
) -> Tuple[SMCParticleApproximation, State_T, Array]:
    ...
    should_resample = log_ess < jnp.log(ess_threshold * x.num_samples)
...
def smc_sampler(
    ...
    CarryType = tuple[SMCStepState[State_T], KeyArray]
    ...
    def smc_step(
        ...
        mcmc_result = vmap(inner_kernel.n_steps, in_axes=(0, None, 0))(
            mcmc_state_post_resampling,
            config.inner_kernel_steps,
            random.split(key, num=samples_i.num_samples),
        )
    ...
    if init_state is None:
        ...
        init_state = init_state.replace(
            particle_approximation=init_state.particle_approximation.replace(
                log_prob=log_p0
            )
        )
    ...
    if config.record_trajectory:
        ...
        _, all_stats = outputs[0].particle_approximation, outputs[1]
        ...
        smoke_result = vmap(type(kernel).one_step, in_axes=(None, 0, None))(
            kernel, init_state.kernel_state, key
        )
...
def _resample_step_sizes(
    step_sizes: Array,
    step_sizes_idxs: Array,
    init_particles: State_T,
    final_particles: State_T,
    key: KeyArray,
) -> Tuple[Array, Array]:
    dists = jnp.sum(jnp.square(init_particles.x - final_particles.x), axis=-1)
    dists_per_idx = vmap(
        lambda idx: jnp.sum(dists * (step_sizes_idxs == idx))
        / jnp.sum(step_sizes_idxs == idx)
    )(jnp.arange(len(step_sizes)))
    weights = dists_per_idx / jnp.sum(dists_per_idx)

    from numpyro import distributions as np_distributions

    _eps = 0.5
    indiv_dists = np_distributions.Uniform(
        low=step_sizes * (1 - _eps),  # pyright: ignore[reportGeneralTypeIssues]
        high=step_sizes * (1 + _eps),  # pyright: ignore[reportGeneralTypeIssues]
    )
    mixing_dist = np_distributions.Categorical(probs=weights)

    step_size_dist = np_distributions.MixtureSameFamily(
        mixing_distribution=mixing_dist, component_distribution=indiv_dists
    )
    key, subkey = random.split(key)
    ...
    key, subkey = random.split(key)
    new_step_size_idxs = random.randint(
        minval=0,
        maxval=len(new_step_sizes),
        shape=(init_particles.x.shape[0],),
        key=subkey,
    )
...
def smc_sampler_adaptive(
    ...
    if jnp.atleast_1d(kernel.get_step_size()).shape[0] == 1:
        ...
        key, subkey = random.split(key)
        step_sizes_idxs = random.randint(
            minval=0, maxval=config.num_step_sizes, key=subkey, shape=(x0.num_samples,)
        )
    else:
    ...
    CarryType = tuple[AdaptiveSMCStepState[State_T], KeyArray]
    ...
    def smc_step(
        ...
        key, subkey = random.split(key)
        mcmc_result = vmap(
            lambda ss, state, k: inner_kernel.set_step_size(ss).n_steps(
                state, config.inner_kernel_steps, k
            ),
            in_axes=(0, 0, 0),
        )(
            carry[0].step_sizes[carry[0].step_sizes_idxs],
            mcmc_state_post_resampling,
            random.split(subkey, num=samples_i.num_samples),
        )

        key, subkey = random.split(key)
        new_step_sizes, new_step_sizes_idxs = _resample_step_sizes(
            carry[0].step_sizes,
            carry[0].step_sizes_idxs,
            mcmc_state_post_resampling,
            mcmc_result.state,
            subkey,
        )
        ...
        smc_stats = AdaptiveSMCStepInfo(
            ...
            did_resample=did_resample,
            dists=jnp.sum(
                jnp.square(mcmc_state_post_resampling.x - mcmc_result.state.x), axis=-1
            ),
        )

        if not config.record_trajectory:
            return (
                AdaptiveSMCStepState(
                    samples_i_post_mcmc,
                    mcmc_result.state,
                    new_step_sizes,
                    new_step_sizes_idxs,
    ...
    if init_state is None:
        init_kernel = config.inner_kernel_factory.build_kernel(log_p0)
        init_state = AdaptiveSMCStepState(
            x0, vmap(init_kernel.init_state)(x0.particles), step_sizes, step_sizes_idxs
        )
    else:
        init_state = init_state.replace(
            particle_approximation=init_state.particle_approximation.replace(
...
class SMC(
    ...
    def run(
        self, key: KeyArray, update_init_state: bool = False
    ) -> Tuple[Self, SMCResults[State_T, Info_T]]:
        assert self.init_state is not None
        assert isinstance(self.init_state, SMCParticleApproximation)
        results = smc_sampler(
            self.log_prob,
            self.init_state,
            self.config,
            key,
            init_state=self._init_smc_state,
        )
        return self, results
    ...
    def run_and_update_init(
        ...
        final_state = cast(
            SMCStepState[State_T],
            tree_map(lambda x: x[-1], results.info.smc_steps_states),
        )

        # some pytree manipultation
        final_state = final_state.replace(
            particle_approximation=final_state.particle_approximation.replace(
                log_prob=results.samples.log_prob
            )
        )

        self = self.replace(init_state=results.samples, _init_smc_state=final_state)
        return self, results


class AdaptiveSMC(
    Generic[Config_T, State_T, Info_T],
    InferenceAlgorithm[AdaptiveSMCConfig[Config_T, State_T, Info_T]],
):
    config: AdaptiveSMCConfig[Config_T, State_T, Info_T]
    ...
    def run(
        self, key: KeyArray, update_init_state: bool = False
    ) -> Tuple[Self, AdaptiveSMCResults[State_T, Info_T]]:
        assert self.init_state is not None
        assert isinstance(self.init_state, SMCParticleApproximation)
        results = smc_sampler_adaptive(
            self.log_prob,
            self.init_state,
            self.config,
            key,
...
class SMCFactory(
    Generic[Config_T, State_T, Info_T],
    InferenceAlgorithmFactory[SMCConfig[Config_T, State_T, Info_T]],
):
...
class AdaptiveSMCFactory(
    Generic[Config_T, State_T, Info_T],
    InferenceAlgorithmFactory[AdaptiveSMCConfig[Config_T, State_T, Info_T]],
):
    def build_algorithm(
        self, log_prob: LogDensity_T
    ) -> AdaptiveSMC[Config_T, State_T, Info_T]:
