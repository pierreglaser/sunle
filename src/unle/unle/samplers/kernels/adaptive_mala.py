...
class AdaptiveMALAState(State):
    ...
    def _update_stats(
        ...
        if update_sigma:
            ...
            new_sigma = new_state.p1(
                new_state.sigma + gamma_n * (alpha - self.target_accept_rate)
            )
        ...
        if update_cov:
            mu = self.get_mu()
            new_mu = new_state.p3((1 - gamma_n) * mu + gamma_n * x)
            if self.has_diagonal_cov:
                new_C = new_state.p2(
                    (1 - gamma_n) * self.get_C() + gamma_n * (x - mu) ** 2
                )
            else:
                new_C = new_state.p2(
                    (1 - gamma_n) * self.get_C() + gamma_n * (jnp.outer(x - mu, x - mu))
                )
...
class PreconditionedMALAProposalDistDiagCov(struct.PyTreeNode):
    ...
    def mean(self, x_cond: Array_T) -> Array_T:
        return x_cond + self.sigma**2 * self.reg_C * grad(self.target_log_prob)(
            x_cond
        )
...
class AdaptiveMALAKernel(
    ...
    def init_state(self, x: Array):
        if self.config.update_cov:
            if self.config.use_dense_cov:
                return AdaptiveMALAState(
                    x, 0, x, jnp.eye(x.shape[0]), jnp.sqrt(self.config.step_size)
                )
            else:
                return AdaptiveMALAState(
                    x, 0, x, jnp.ones((x.shape[0],)), jnp.sqrt(self.config.step_size)
                )
        else:
            return AdaptiveMALAState(x, 0, None, None, jnp.sqrt(self.config.step_size))

    def get_proposal_dist(
        self, x: AdaptiveMALAState
    ) -> Union[
        PreconditionedMALAProposalDist,
        PreconditionedMALAProposalDistNoCov,
        PreconditionedMALAProposalDistDiagCov,
    ]:
        if self.config.update_cov:
    ...
    def one_step(
        self, x: AdaptiveMALAState, key: KeyArray
    ) -> Result[AdaptiveMALAState, AdaptiveMALAInfo]:
        key_proposal, key_accept, key = random.split(key, num=3)
        q = self.get_proposal_dist(x)
...
class AdaptiveMALAKernelFactory(
    KernelFactory[AdaptiveMALAConfig, AdaptiveMALAState, AdaptiveMALAInfo]
):
