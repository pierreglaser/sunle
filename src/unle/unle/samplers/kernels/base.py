...
class Kernel(
    ...
    @abc.abstractmethod
    def create(
        cls: Type[Self], target_log_prob: LogDensity_T, config: Config_T
    ) -> Self:
    ...
    @abc.abstractmethod
    def one_step(self, x: State_T, key: KeyArray) -> Result[State_T, Info_T]:
        raise NotImplementedError

    def n_steps(self, x: State_T, n: int, key: KeyArray) -> Result[State_T, Info_T]:
        result = self.one_step(x, fold_in(key, 0))

        def body_fun(
            i: int, result: Result[State_T, Info_T]
...
class MHKernel(
    Kernel[Config_T, State_T, Info_T],
    ...
    @abc.abstractmethod
    def sample_from_proposal(self, key: KeyArray, x: State_T) -> State_T:
        raise NotImplementedError
    ...
    def one_step(self, x: State_T, key: KeyArray) -> Result[State_T, Info_T]:
        ...
        proposal = self.sample_from_proposal(key_proposal, x)
...
class TunableKernel(
    MHKernel[TunableConfig_T, State_T, Info_T],
    Generic[TunableConfig_T, State_T, Info_T],
):
...
class KernelFactory(Generic[Config_T, State_T, Info_T_co], struct.PyTreeNode):
    config: Config_T
    kernel_cls: Type[Kernel[Config_T, State_T, Info_T_co]] = struct.field(
        pytree_node=False
    )

    def build_kernel(
        self, log_prob: LogDensity_T
    ) -> Kernel[Config_T, State_T, Info_T_co]:
        return self.kernel_cls(log_prob, self.config)


class MHKernelFactory(KernelFactory[Config_T, State_T, Info_T_co]):
    config: Config_T
    kernel_cls: Type[MHKernel[Config_T, State_T, Info_T_co]] = struct.field(
        pytree_node=False
    )

    def build_kernel(
        self, log_prob: LogDensity_T
    ) -> MHKernel[Config_T, State_T, Info_T_co]:
...
class TunableMHKernelFactory(KernelFactory[TunableConfig_T, State_T, Info_T_co]):
    ...
    kernel_cls: Type[TunableKernel[TunableConfig_T, State_T, Info_T_co]] = struct.field(
        pytree_node=False
    )

    def build_kernel(
        self, log_prob: LogDensity_T
    ) -> TunableKernel[TunableConfig_T, State_T, Info_T_co]:
        return self.kernel_cls.create(log_prob, self.config)

    def set_step_size(self, step_size: Array_T):
        return self.replace(config=self.config.set_step_size(step_size))

    def get_step_size(self):
        return self.config.get_step_size()

    def set_inverse_mass_matrix(self, C: Array_T):
        return self.replace(config=self.config.set_inverse_mass_matrix(C))

    def get_inverse_mass_matrix(self):
        return self.config.get_inverse_mass_matrix()
